...
def get_translation(word: str) -> list:
    ...
    response = requests.post(url, data={
        "xajax": "XTranslate",
        "xajaxargs[]": f"<xjxquery><q>word={quote(word)}&word_enc=&lang=1&nikud=1&transcription=1</q></xjxquery>"
    })
    xml_soup = BeautifulSoup(response.text, "xml")
    html_soup = BeautifulSoup(xml_soup.findAll("cmd")[-1].text, "html.parser")
    divs = html_soup.findAll("div", class_=lambda x: x and ("cycle1" in x or "cycle-1" in x))
    for pare in divs:
